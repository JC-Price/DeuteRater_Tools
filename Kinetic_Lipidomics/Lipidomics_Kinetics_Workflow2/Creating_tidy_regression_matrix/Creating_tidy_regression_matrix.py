...
METRIC_TOKENS = [
    ...
    "DR_Abundance_SE", 
    "abundance_se",   # {group}_DR_Abundance_SE or DR_Abundance_SE_{group}
...
def show_basic_filter_window(root: tk.Tk) -> dict:
    ...
    tk.Entry(frm, textvariable=amax_var, width=10).grid(
    ...
    # -------------------------------
    # NEW: Max Abundance rate (K)
    # -------------------------------
    tk.Label(frm, text="Maximum Abundance rate (K):").grid(row=3, column=0, sticky="w", pady=(10, 0))
    kmax_var = tk.StringVar(value=str(settings["max_rate"]))
    tk.Entry(frm, textvariable=kmax_var, width=10).grid(
        row=3, column=1, sticky="w", padx=(6, 0), pady=(10, 0)
    )
    tk.Label(frm, text="references: Abundance rate_<sample_token>").grid(
        row=3, column=2, sticky="w", padx=(10, 0), pady=(10, 0)
    )
    ...
    ).grid(row=4, column=0, columnspan=3, sticky="w", pady=(12, 12))
    ...
    def do_continue():
        ...
        # Validate K max
        try:
            kmax = float(kmax_var.get())
            if kmax <= 0:
                raise ValueError
        except Exception:
            messagebox.showerror("Invalid maximum Abundance rate", "Enter a number > 0 (e.g., 5).")
            return

        # Store validated settings
        settings["r2_cutoff"] = val
        settings["max_asymptote"] = amax
        settings["max_rate"] = kmax          # NEW
...
def apply_basic_filters_wide(
    ...
    rate_max: float | None = None,          # ✅ NEW: cap for Abundance rate (e.g., 5.0)
) -> pd.DataFrame:
    """
    ...
    Rules implemented per sample token:
      1) Gate by R² < r2_cutoff  (column: 'Abundance R2_<sample>')
      2) Gate by Abundance asymptote > asymptote_max  (column: 'Abundance asymptote_<sample>')
      3) Gate by Abundance rate > rate_max            (column: 'Abundance rate_<sample>')  ✅ NEW

      If ANY of the above fail, we set BOTH:
        - 'Abundance rate_<sample>' and
        - 'Abundance asymptote_<sample>'
      to NaN (unless drop_rows=True, in which case we drop entire rows).

      4) Optionally enforce n_L (n_value) < nH:
         - Compare 'n_value_<sample>' with 'nH'
         - If violated, set 'n_value_<sample>' to NaN (or drop rows if drop_rows=True).
    """
    out = df.copy()
    ...
    # From detected per-sample metric columns
    ...
    # Also infer from dataframe columns
    ...
    # -----------------------------
    # 1) R² / Asymptote / Rate gating (combined exclusion)
    # -----------------------------
    if not samples:
        ...
        for sample in sorted(samples):
            ...
            # ✅ Rate gating (only if user provided a threshold)
            if rate_max is not None and rate_col in out.columns:
                rate = pd.to_numeric(out[rate_col], errors="coerce")
                rate_bad = rate.notna() & (rate > rate_max)

            combined_bad = r2_bad | asym_bad | rate_bad
            ...
            if drop_rows:
                ...
                msg = (
                    ...
                    f"(R² bad: {n_r2}, asymptote big: {n_asym}, rate big: {n_rate}) {aid_str}"
                )
                if print_summary:
                    print(msg)
                tell(msg)
                ...
                changed_targets = []
                for target in (rate_col, asym_col):
                    if target in out.columns:
                        ...
                        if print_summary:
                            print(
                                f"{target} set to NaN if "
                                f"(R²<{r2_cutoff} OR asym>{asymptote_max} OR rate>{rate_max}) "
                                f"(newly NaN: {newly_set}) {aid_str}"
                            )

                if changed_targets:
                    tell(
                        f"R²/asymptote/rate gating for sample '{sample}': "
                        f"{n_combined} rows affected (R² bad: {n_r2}, asymptote big: {n_asym}, rate big: {n_rate}) -> "
...
def main() -> None:
    ...
    # --- NEW: Backfill Ontology from first token of Alignment ID if blank ---
    # Create Ontology if it doesn't exist
    if "Ontology" not in df.columns:
        df["Ontology"] = np.nan
    
    # Build a "blank" mask = NaN or empty/whitespace
    onto_blank = df["Ontology"].isna() | (df["Ontology"].astype(str).str.strip() == "")
    
    # First token from space-delimited Alignment ID (robust to NaN)
    first_token = (
        df["Alignment ID"]
        .astype(str)
        .str.strip()
        .str.split()                # split on any whitespace
        .str[0]                     # take first token
        .where(df["Alignment ID"].notna(), other=np.nan)
    )
    
    # Fill only where Ontology is blank
    df.loc[onto_blank, "Ontology"] = first_token
    # --- END NEW ---
    ...
    # General, robust: first non-null per column across duplicates
    df = (df.groupby("Alignment ID", as_index=False)
            .agg(lambda s: next((x for x in s if pd.notna(x)), pd.NA)))
    ...
    df = apply_basic_filters_wide(
        ...
        asymptote_max=float(settings["max_asymptote"]),
        rate_max=float(settings["max_rate"]),   # ✅ pass the cap here
    )
