...
def get_user_settings(parent):
    ...
    # IMPORTANT: bind variables to this Toplevel (gui)
    vars_dict = {k: tk.StringVar(master=gui, value=str(v)) for k, v in defaults.items()}

    show_common_var = tk.BooleanVar(master=gui, value=True)
    vars_dict["show_common_components"] = show_common_var
    
        
    use_zero_zero_var = tk.BooleanVar(master=gui, value=True)
    vars_dict["use_zero_zero"] = use_zero_zero_var
    
    
    # --- add next to your existing BooleanVars in get_user_settings ---
    include_ether_var = tk.BooleanVar(master=gui, value=False)   # default: exclude ethers
    vars_dict["include_ether"] = include_ether_var
    ...
    # Make window and columns expand nicely
    ...
    ).grid(column=0, row=0, columnspan=4, pady=(0, 15), sticky="w")

    ttk.Label(frm, text="Metric").grid(column=0, row=1, sticky="w")
    ttk.Label(frm, text="Low").grid(column=1, row=1, sticky="w")
    ttk.Label(frm, text="High").grid(column=2, row=1, sticky="w")
    ...
    for label, low_key, high_key in rows:
        ...
        ttk.Entry(frm, textvariable=vars_dict[low_key], width=10).grid(
            column=1, row=current_row, sticky="w"
        )
        ttk.Entry(frm, textvariable=vars_dict[high_key], width=10).grid(
            column=2, row=current_row, sticky="w"
        )
        current_row += 1
    ...
    # ----------------------------
    ttk.Label(frm, text="Component min % (nL deconvolution):").grid(
        column=0, row=current_row, sticky="e", pady=10
    )
    ...
    # ----------------------------
    ttk.Label(frm, text="Monte Carlo iterations (per genotype):").grid(
        column=0, row=current_row, sticky="e", pady=10
    )
    ttk.Entry(frm, textvariable=vars_dict["mc_iters"], width=10).grid(
        column=1, row=current_row, sticky="w"
    )
    current_row += 1
    ...
    ttk.Checkbutton(
        frm,
        text="Treat 0:0 as a component?",
        variable=use_zero_zero_var
    ).grid(
        column=0, row=current_row, columnspan=3, sticky="w", pady=(10, 10)
    )
    current_row += 1
    ...
    # --- NEW: GUI toggle for ethers ---
    ttk.Checkbutton(
        frm,
        text="Include ether species (O-/P-; Ether*)?",
        variable=include_ether_var
    ).grid(column=0, row=current_row, columnspan=3, sticky="w", pady=(0, 10))
    current_row += 1
    ...
    def submit():
        ...
        try:
            ...
            # Optional: show an error dialog, but keep it lightweight
            tk.messagebox.showerror("Input error", f"Could not parse one of the values:\n{e}")
            return
        ...
        gui.destroy()  # close the window; wait_window will return

    ttk.Button(frm, text="OK", command=submit).grid(
        column=0, row=current_row, columnspan=3, pady=15, sticky="w"
    )

    # IMPORTANT: do NOT start a nested mainloop here.
    # Block until the window is destroyed, then return settings.
    parent.wait_window(gui)
...
# === NEW ===
def plot_component_barplot_asymptote(
    group_summaries_asyn,
    iter_dir,
    tag,
    top_n=30,
    show_common=True
    ...
    """
    Plot per-group Asymptote component labeling fractions (Median with 95% CI),
    on a fixed y-axis [0, 1], no literature overlay.
    """
    groups_list = list(group_summaries_asyn.keys())
    if not groups_list:
        print("⚠️ No Asymptote summaries to plot.")
        return

    # Component universe
    comp_sets = [set(group_summaries_asyn[g].index) for g in groups_list]
    all_components = (sorted(set.intersection(*comp_sets)) if show_common
                      else sorted(set().union(*comp_sets)))
    if not all_components:
        print("⚠️ No components after intersection/union rule for Asymptote plot.")
        return

    # Build matrices
    n_comp, n_grp = len(all_components), len(groups_list)
    ...
    for j, g in enumerate(groups_list):
        s = group_summaries_asyn[g].reindex(all_components)
        med_mat[:, j] = s["Median"].to_numpy(float)
        lo_mat[:, j]  = s["Lower"].to_numpy(float)
        hi_mat[:, j]  = s["Upper"].to_numpy(float)

    # Select top-N by variability and reorder FA-first
    ...
    # Plot
    plt.figure(figsize=(max(10, 0.35 * len(comps)), 6), dpi=150)
    ...
    for j, g in enumerate(groups_list):
        med = med_mat[:, j]; low = lo_mat[:, j]; high = hi_mat[:, j]
        ok  = np.isfinite(med) & np.isfinite(low) & np.isfinite(high)
        if not np.any(ok):
            continue

        err  = np.vstack([med[ok] - low[ok], high[ok] - med[ok]])
        xpos = x[ok] + offsets[j]

        plt.bar(xpos, med[ok], width=bar_width * 0.85,
                color=colors[j % len(colors)], edgecolor="black", zorder=2,
                label=str(g))
        plt.errorbar(xpos, med[ok], yerr=err, fmt="none",
                     ecolor="black", elinewidth=1.2, capsize=6, capthick=1.2, zorder=5)

    plt.xticks(x, comps, rotation=60, ha="right")
    plt.ylabel(r"Estimated component labeling fraction ($A_i$)")
    plt.title(f"Asymptote component contributions (0–1)\n{tag}")
    ...
    plt.tight_layout()
    outpath = os.path.join(iter_dir, f"bar_components_Asymptote_{tag}.svg")
    plt.savefig(outpath, dpi=300)
    plt.close()
    print(f"✅ Saved Asymptote barplot (0–1 scale): {outpath}")
...
def plot_component_barplot(
    ...
    outpath = os.path.join(iter_dir, f"bar_components_{tag}.svg")
    plt.savefig(outpath, dpi=300)
    plt.close()
...
# Wire GUI toggle into parsing: include_ether -> exclude_ether flag for the parser
include_ether = settings.get("include_ether", False)
exclude_ether = not include_ether
...
use_zero_zero = settings["use_zero_zero"]
DROP_PAD_TOKEN = not use_zero_zero
PAD_TOKEN = "0:0"
