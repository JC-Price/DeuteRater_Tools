...
for z_k_cut in SE_K_RANGE:
    for z_asyn_cut in SE_ASYN_RANGE:
        for z_n_cut in SE_N_RANGE:
            ...
            after = len(df_n)
            print(f"Filtering n$_L$ > 10 kept {after}/{before}")

            if after == 0:
                open(os.path.join(iter_dir, f"EMPTY_n$_L$_{tag}.txt"), "w").write(
                    "No peptides with n$_L$ > 10.\n"
                )
                results_summary.append({
                    "z_k_cut": z_k_cut,
                    "z_Asyn_cut": z_asyn_cut,
            ...
            for grp, summary in group_summaries.items():
                ...
                sub["predicted_n$_L$"] = predicted
                sub = sub.dropna(subset=["n_value","predicted_n$_L$"])
            
                x = sub["predicted_n$_L$"].to_numpy(float)
                ...
                # ---- Passing–Bablok conformity plot (WITH mean residual shading) ----
                x_fit = np.linspace(x.min(), x.max(), 300)
                y_fit = slope * x_fit + intercept
                y_low = slope_ci[0] * x_fit + intercept_ci[0]
                y_high = slope_ci[1] * x_fit + intercept_ci[1]
                
                # Compute residuals relative to the Passing–Bablok fit
                y_pred = slope * x + intercept
                residuals = y - y_pred
                mean_res = float(np.mean(residuals))   # <-- MATCHES YOUR OTHER SCRIPT
                ...
                # Fit line
                plt.plot(x_fit, y_fit, "r-", label="Passing–Bablok fit")
                
                # Confidence interval
                plt.fill_between(x_fit, y_low, y_high,
                                 color="lightgray", alpha=0.5,
                                 label="95% CI")
                
                # Mean residual shading (CONFORMITY shading)
                plt.fill_between(
                    x_fit,
                    y_fit - mean_res,
                    y_fit + mean_res,
                ...
                plt.plot(x_fit, x_fit, "--", color="gray", label="Identity")
                
                plt.xlabel("Predicted n$_L$")
                plt.ylabel("Measured n$_L$")
                plt.title(
                    f"{grp} Passing–Bablok Conformity\n"
                    f"y = {slope:.3f}x + {intercept:.3f} | n = {len(x)} | R² = {r2:.3f}\n"
                    f"Mean(residuals) = {mean_res:.3f} n$_L$\n"
                    f"{tag}"
                )
                ...
                plt.savefig(os.path.join(iter_dir, f"PB_{grp}_{tag}.svg"), dpi=300)
                plt.close()
                ...
                plt.grid(alpha=0.3)
                plt.tight_layout()
                plt.savefig(os.path.join(iter_dir, f"BA_{grp}_{tag}.svg"), dpi=300)
                plt.close()
            
            
                # ----------------------------
                # Barplot empirical vs theoretical (using MEAN DIFFERENCE, not MAD)
                # ----------------------------
                ...
                # Precompute theoretical values aligned to ALL_AAS
                theory_vals = np.array([theoretical_nL_master.get(aa, np.nan) for aa in ALL_AAS])
                
                # Masks for essential vs non-essential by AA index
                mask_E  = np.array([aa in ESSENTIAL_AAS for aa in ALL_AAS])
                mask_NE = ~mask_E
                
                summary_lines = []
                
                # Will store the FIRST group's values for the legend
                legend_MEAN_E  = None
                legend_MEAN_NE = None
                ...
                for i, grp in enumerate(groups_list):
                    ...
                    plt.bar(
                        x_idx + i*width, med, width=width, color=bar_colors,
                        yerr=err, capsize=3, edgecolor="black", label=f"{grp}"
                    )
                
                    # ---- Compute MEAN DIFFERENCE (empirical - theoretical) ----
                    diff = med - theory_vals
                
                    mean_diff_E  = np.nanmean(diff[mask_E])  if np.any(mask_E)  else np.nan
                    mean_diff_NE = np.nanmean(diff[mask_NE]) if np.any(mask_NE) else np.nan
                
                    # Store legend values ONCE (from first group)
                    if legend_MEAN_E is None:
                        legend_MEAN_E  = mean_diff_E
                        legend_MEAN_NE = mean_diff_NE
                
                    # Add into the summary line for the title
                    summary_lines.append(
                        f"{grp}: mean diff(E)={mean_diff_E:.3f}, mean diff(NE)={mean_diff_NE:.3f}"
                    )
                
                # --- Theoretical bars ---
                plt.bar(
                    x_idx + len(groups_list)*width, theory_vals,
                    width=width, color="lightgray", edgecolor="black",
                    hatch="///", label="Theoretical"
                )
                
                plt.xticks(x_idx + (n_groups - 1)/2 * width, ALL_AAS)
                plt.ylabel("n$_L$ per AA")
                
                # ---- Title ----
                title_stats = " | ".join(summary_lines) if summary_lines else ""
                plt.title(f"Empirical vs Theoretical AA n$_L$ values\n{title_stats}\n{tag}")
                
                # ---- FIXED LEGEND ----
                import matplotlib.patches as mpatches
                
                patch_NE = mpatches.Patch(
                    color=colors[0],
                    label=f"Estimated Non-Essential (mean diff = {legend_MEAN_NE:.3f})"
                )
                patch_E = mpatches.Patch(
                    color="red",
                    label=f"Estimated Essential (mean diff = {legend_MEAN_E:.3f})"
                )
                patch_T = mpatches.Patch(
                    facecolor="lightgray",
                    edgecolor="black",
                    hatch="///",
                    label="Theoretical"
                ...
                plt.legend(handles=[patch_NE, patch_E, patch_T], fontsize=8)
                
                plt.grid(axis="y", alpha=0.3)
                plt.tight_layout()
                plt.savefig(os.path.join(iter_dir, f"bar_empirical_vs_theoretical_{tag}.svg"), dpi=300)
                plt.close()
