...
def _build_tab_with_enable(parent, title: str, plot_type: str,
    ...
    if plot_type.lower() == "homeostasis":
        # Single filter editor for homeostasis
        box2 = ttk.LabelFrame(frame, text="Filters", padding=8)
        box2.grid(row=2, column=0, sticky="nsew", padx=2, pady=(0,0))
        frame.grid_rowconfigure(2, weight=1)

        var_homeo = tk.BooleanVar(value=False)

        row = ttk.Frame(box2)
        row.grid(row=0, column=0, sticky="ew", pady=(0,4))

        ttk.Checkbutton(row, variable=var_homeo).pack(side="left")
        ttk.Label(row, text="Homeostasis Filters").pack(side="left", padx=(6,0))

        txt_homeo = tk.Text(box2, height=8, width=60)
        txt_homeo.grid(row=1, column=0, sticky="nsew")
        txt_homeo.configure(state="disabled")

        box2.grid_rowconfigure(1, weight=1)
        box2.grid_columnconfigure(0, weight=1)

        sections = [
            ("Homeostasis", var_homeo, txt_homeo, None)
        ]
...
def dump_gui_state(state_registry: dict) -> dict:
    ...
    if settings_tab is not None and hasattr(settings_tab, "_norm_method_var"):
        method_map = {
            "None (raw)": "none",
            "mTIC": "mtic",
            "Standards (requires baseline)": "standards",
            "Quantile": "quantile",
        }
        sel = settings_tab._norm_method_var.get()
        method = method_map.get(sel, "none")
        baseline = settings_tab._baseline_entry.get().strip() if hasattr(settings_tab, "_baseline_entry") else None
        config["Settings"] = {
            "normalization": {
                "method": method,
                "baseline": baseline if method == "standards" else None,
            }
        }
...
def apply_gui_state(config: dict, state_registry: dict):
    ...
    if settings_tab is not None and hasattr(settings_tab, "_norm_method_var"):
        ...
        baseline = norm_cfg.get("baseline", "")
        rev_map = {
            "none": "None (raw)",
            "mtic": "mTIC",
            "standards": "Standards (requires baseline)",
            "quantile": "Quantile",
        }
        # Back-compat fallback from old booleans if method is missing/unknown
        if method not in rev_map:
            old_mtic = bool(settings_cfg.get("perform_abundance_normalization", False))
            old_std  = bool(settings_cfg.get("use_standards_normalization", False))
            chosen = "Standards (requires baseline)" if old_std else ("mTIC" if old_mtic else "None (raw)")
            settings_tab._norm_method_var.set(chosen)
        else:
            settings_tab._norm_method_var.set(rev_map[method])
    
        # Baseline
        if hasattr(settings_tab, "_baseline_entry"):
            settings_tab._baseline_entry.configure(state="normal")
            settings_tab._baseline_entry.delete(0, "end")
            settings_tab._baseline_entry.insert(0, baseline or "A3")
    
        # Re-apply enable/disable logic
        if hasattr(settings_tab, "_apply_baseline_state"):
            settings_tab._apply_baseline_state()
...
def _build_settings_tab(parent, state_registry) -> ttk.Frame:
    ...
    # -------------------
    # Normalization method (single choice)
    # -------------------
    norm_frame = ttk.LabelFrame(frame, text="Normalization", padding=10)
    norm_frame.pack(fill="x", anchor="w", pady=(6, 10))

    ttk.Label(norm_frame, text="Choose one normalization method:").grid(row=0, column=0, sticky="w")
    norm_method_var = tk.StringVar(value="None (raw)")
    norm_method = ttk.Combobox(
        norm_frame,
        textvariable=norm_method_var,
        values=[
    "None (raw)",
    "mTIC",
    "Standards (requires baseline)",
    "Quantile",
],
        state="readonly",
        width=30
    )
    norm_method.grid(row=0, column=1, padx=(8, 0), sticky="w")

    # Baseline (enabled only for 'Standards')
    baseline_row = ttk.Frame(norm_frame)
    baseline_row.grid(row=1, column=0, columnspan=2, sticky="ew", pady=(8, 0))
    ttk.Label(
        baseline_row,
        text="Baseline group (only for 'Standards'):"
    ).pack(side="left", padx=(0, 8))
    baseline_entry = ttk.Entry(baseline_row, width=30)
    baseline_entry.insert(0, "A3")
    baseline_entry.pack(side="left", fill="x", expand=True)

    def _apply_baseline_state(*_):
        wants_baseline = (norm_method_var.get() == "Standards (requires baseline)")
        baseline_entry.state(["!disabled"] if wants_baseline else ["disabled"])

    norm_method_var.trace_add("write", _apply_baseline_state)
    _apply_baseline_state()

    # --- YAML status + buttons (existing code block below stays as-is) ---
    status_var = tk.StringVar(value="No config loaded")
    btn_row = ttk.Frame(frame)
    btn_row.pack(anchor="w", pady=(0, 8))
    btn_import = ttk.Button(btn_row, text="Import YAML", width=20)
    ...
    # Attach state to Settings tab
    frame._norm_method_var = norm_method_var
    frame._baseline_entry = baseline_entry
    frame._apply_baseline_state = _apply_baseline_state

    # Auto-load default YAML (existing behavior kept)
    default_path = Path(__file__).parent / "post_hoc_default.yaml"
    if YAML_AVAILABLE and default_path.exists():
        try:
            cfg = load_yaml_config(default_path)
