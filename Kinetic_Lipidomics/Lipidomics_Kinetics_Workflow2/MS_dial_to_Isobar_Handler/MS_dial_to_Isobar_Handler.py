...
def modify_data_frame(df, remove_PQCs = False):
    ...
    bad_flag = new_df['Metabolite name'].str.contains(
        r'no MS2:|low score:', case=False, na=False
    )
    
    is_d7 = new_df['Metabolite name'].str.contains(
        r'\bd7\b', case=False, na=False
    )
    
    drop_conditions = bad_flag & ~is_d7
