...
# Alignment ID, Ontology, metric, value, se, qcm, ci_lower, ci_upper, genotype
#
# QC RULE (IMPORTANT):
...
expected = ["Sequence", "Ontology", "metric", "value", "se",
            "qcm", "ci_lower", "ci_upper", "group_name"]
missing = [c for c in expected if c not in df.columns]
if missing:
    raise SystemExit(f"ERROR: Missing expected columns: {missing}")

# Ensure numeric columns
for col in ["value", "se", "qcm", "ci_lower", "ci_upper"]:
    df[col] = pd.to_numeric(df[col], errors="coerce")

# Fix reversed CI bounds (your file has some)
swap_mask = np.isfinite(df["ci_lower"]) & np.isfinite(df["ci_upper"]) & (df["ci_lower"] > df["ci_upper"])
if swap_mask.any():
    lo = df.loc[swap_mask, "ci_upper"].copy()
    hi = df.loc[swap_mask, "ci_lower"].copy()
    df.loc[swap_mask, "ci_lower"] = lo
    df.loc[swap_mask, "ci_upper"] = hi
    print(f"Swapped ci_lower/ci_upper for {swap_mask.sum()} rows where bounds were reversed.")

print("\nUsing QC metric column: qcm")
...
# ----------------------------
# CI normalization and SE fallback
# ----------------------------
Z95 = 1.96

def ci_to_sd(lo, med, hi):
    if not (np.isfinite(lo) and np.isfinite(hi)):
        return np.nan
    return float(max(1e-8, (hi - lo) / (2.0 * Z95)))

sd_from_ci_vec = np.array([
    ci_to_sd(lo, v, hi) if (np.isfinite(lo) and np.isfinite(hi)) else np.nan
    for lo, v, hi in zip(df["ci_lower"], df["value"], df["ci_upper"])
], dtype=float)

df["sd_pref"] = np.where(np.isfinite(df["se"]) & (df["se"] > 0), df["se"], sd_from_ci_vec)
df["sd_pref"] = np.where(np.isfinite(df["sd_pref"]), df["sd_pref"], 1e-6)

df["ci_lower"] = df["value"] - Z95 * df["sd_pref"]
df["ci_upper"] = df["value"] + Z95 * df["sd_pref"]
df["ci_width"] = 2.0 * Z95 * df["sd_pref"]
df["frac_uncert"] = df["ci_width"] / df["value"].abs().replace(0, np.nan)
...
def run_jackknife_mc_allowneg(
    ...
    sd_from_ci = np.array([ci_to_sd(lo, m, hi) for lo, m, hi in zip(lo_bounds, medians, hi_bounds)])
    ...
    for b in outer_iter:
        ...
        for _ in inner_iter:
            sampled = np.array([sample_from_sd(med_sub[i], sd_sub[i], rng=rng) for i in range(len(med_sub))])
            try:
    ...
    results = np.asarray(all_results) if len(all_results) > 0 else np.empty((0, n_aa))
...
for z_k_cut in SE_K_RANGE:
    for z_asyn_cut in SE_ASYN_RANGE:
        for z_n_cut in SE_N_RANGE:
            ...
            # Save QC stats
            summary_qc = df_iter.groupby("metric")[["ci_width","frac_uncert"]].agg(["mean","std","min","max"])
            ...
            for col in ["n_value","n_val_lower_margin","n_val_upper_margin","n_val_se"]:
            ...
            for grp in sorted(df_n["group_name"].unique()):
                ...
                lo_bounds = sub["n_val_lower_margin"].to_numpy(float)
                hi_bounds = sub["n_val_upper_margin"].to_numpy(float)
                ses       = sub["n_val_se"].to_numpy(float)

                results_obs, diag = run_jackknife_mc_allowneg(
                    A, medians, lo_bounds, hi_bounds, ses,
                    num_jackknifes=20, mc_per_boot=200,
                    seed=0, allow_negative=True,
                    l2_reg=1e-8
                )
                diag_all[grp] = diag
