...
# PARSING MODULE (4. parsing)
#
# This module converts lipid identifiers (Alignment IDs + Ontology)
# into their **component parts**:
#   - Fatty acid chains (FA tokens, e.g. "16:0", "18:1")
#   - Structural head groups (e.g. Glycerol, Choline, Serine)
#
# Functions:
#   _extract_fa_tokens()
#       - Extracts FA chains from lipid identifiers.
#       - Handles ether prefixes, lysos, DG padding, and QC printout.
#       - Padding rules:
#           * Lyso classes → 2 total chains (pad with "0:0" if needed).
#           * DGs → padded to 3 chains (even though chemically 2 FAs),
#             so they fit the same design matrix dimension as TGs.
#           * TGs → always have 3 chains in input → no padding needed.
#
#   _components_from_row()
#       - Builds the full component list for one lipid row.
#       - Includes FA chains (via _extract_fa_tokens) and adds
#         head-group–specific structural components.
#
# Step-by-step in _extract_fa_tokens():
#   (A) Define lipid class groups (main_two_fa, lyso_two_fa, DG tri_fa, require_species).
#   (B) Drop adduct/RT annotations from the ID.
#   (C) Enforce species requirement: DGs and phospholipids require "|".
#   (D) If "|" present, use only the part after the first "|".
#   (E) Normalize ether prefixes by removing "P-" and "O-".
#   (F) Extract FA tokens with regex.
#   (G) Pad with "0:0" if class requires more tokens than provided.
#   (H) Print QC (original ID → normalized form).
#   (I) Return FA tokens (or None if none found).
#
# Step-by-step in _components_from_row():
#   (A) Skip row if ontology not in restrict_classes or excluded (ether).
#   (B) Extract FA tokens via _extract_fa_tokens.
#   (C) Start component list with FA tokens.
#   (D) Add structural components depending on ontology:
#         - PC/LPC → Glycerol + Choline
#         - PE/LPE → Glycerol + Ethanolamine
#         - PS/LPS → Serine
#         - PI/LPI → Inositol
#         - PG/LPG → two × Glycerol
#         - DG/TG  → Glycerol
#   (E) Return the component list.
# ================================================================
