...
def standards_median_ratio_factor(df, num_med, den_med, standards_col="Standards", tiny=1e-9):
    ...
    factor = 2 ** np.nanmedian(log2_ratio)
    return float(factor)
...
def _rate_t_p_from_stats(row, exp: str, ctl: str):
    ...
    """
    req = [
        f'Abundance rate_{exp}',  f'Abundance std_error_{exp}',  f'Abundance num_measurements_{exp}',
        f'Abundance rate_{ctl}',  f'Abundance std_error_{ctl}',  f'Abundance num_measurements_{ctl}',
    ]
    if not all(c in row.index for c in req):
        return (np.nan, np.nan)

    m1 = row[f'Abundance rate_{exp}']
    se1 = row[f'Abundance std_error_{exp}']
    n1 = row[f'Abundance num_measurements_{exp}']

    m2 = row[f'Abundance rate_{ctl}']
    se2 = row[f'Abundance std_error_{ctl}']
...
def _plain_abn_rep_cols(df: pd.DataFrame, ident: str) -> list[str]:
    ...
    deny = ('dr', 'median', 'lincorr', 'fc', 'log2', 'rate', 'asymptote',
            'std', 'stderr', 'std_error', 'num', 'count',
            'p_value', 'p-', 't_', 'tstat')
    cols = []
    for c in df.columns:
        cl = c.lower()
...
class Experiment:
    ...
    def process_csv(self):
        ...
        try:
            ...
            # ------------------------------------------------------------
            exp_med = f"{exp}_abundance_median"
            ctl_med = f"{ctl}_abundance_median"
            
            combined[exp_med] = combined[exp_cols_used].median(axis=1, skipna=True)
            combined[ctl_med] = combined[ctl_cols_used].median(axis=1, skipna=True)
            
            if use_baseline:
                baseline_med = f"{self.baseline}_abundance_median"
                combined[baseline_med] = combined[base_cols_used].median(axis=1, skipna=True)
            
            # ------------------------------------------------------------
            # 5. Optional standards-based global scaling (purely additive)
            # ------------------------------------------------------------
            if self.normalize_by_standards:
                ...
                # Recompute medians on SAME columns
                combined[exp_med] = combined[exp_cols_used].median(axis=1, skipna=True)
                combined[ctl_med] = combined[ctl_cols_used].median(axis=1, skipna=True)
                combined[baseline_med] = combined[base_cols_used].median(axis=1, skipna=True)
            ...
            combined["FC_abn"] = combined[exp_med] / combined[ctl_med]
            ...
            try:
                ...
                if not std_df.empty:
                    ...
                    if exp_med not in std_df.columns or ctl_med not in std_df.columns:
                        raise ValueError("Abundance medians not found for Standards diagnostic.")
            
                    std_df = std_df.replace([np.inf, -np.inf], np.nan).dropna(
                        subset=[exp_med, ctl_med]
                    )
            
                    if not std_df.empty:
                        log2fc = np.log2(std_df[exp_med] / std_df[ctl_med])
            ...
            seE = pd.to_numeric(combined.get(f'Abundance std_error_{exp}'), errors='coerce')
            seC = pd.to_numeric(combined.get(f'Abundance std_error_{ctl}'), errors='coerce')
            ...
            seA_e = pd.to_numeric(combined.get(f'Abundance std_error_A_{exp}'), errors='coerce')
            seA_c = pd.to_numeric(combined.get(f'Abundance std_error_A_{ctl}'), errors='coerce')
